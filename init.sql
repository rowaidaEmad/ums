...
CREATE TABLE entities (
    ...
    entity_type ENUM('user','course','section','enrollment','parent_link','request','message','room_schedule','announcement') NOT NULL,
...
('message','body','text'),
-- room_schedule
('room_schedule','slot_date','string'),
('room_schedule','start_time','string'),
('room_schedule','end_time','string'),
('room_schedule','room_number','int'),
('room_schedule','course_id','int'),
('room_schedule','section_id','int'),
-- announcement
...
/* ============================================================
   SCHEDULING (Manual bookings) - Now EAV-based
   ============================================================ */

-- Room Schedule view (EAV-based)
...
FROM entities e
LEFT JOIN eav_values v ON v.entity_id = e.id
LEFT JOIN eav_attributes a ON a.id = v.attribute_id AND a.entity_type='room_schedule'
WHERE e.entity_type='room_schedule'
GROUP BY e.id;
...
-- Attributes already inserted above
